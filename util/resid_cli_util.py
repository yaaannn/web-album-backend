...
class RedisCli:
    def __init__(self) -> None:
        """初始化"""
        self._connect_url = settings.CACHES["redis_cli"]["LOCATION"]
        self.current_db = int(self._connect_url[-1])
        self.coon = redis.Redis(
            connection_pool=redis.ConnectionPool().from_url(self._connect_url)
        )
...
class RedisHash(object):
    ...
    def __repr__(self) -> str:
        to_dict = (
            {
                key: self.get(key)
                for key in map(lambda x: x.decode(), self._conn.hkeys(self._data_key))
            }
...
if __name__ == "__main__":
    ...
    print(redis_cli.setdefault("a", 123))
